...
public class Knapsack 
{
	...
	public long calculateMaxValueOfKnapsack()
	{
		...
		//
		//for(int index = 0;index<knapsackSize+1;index++)
		...
		for(int index = 0;index<knapsackSize+1;index++)
			optimizedKnapsackCache[0][index] = 0;
		...
		for(int i=1;i<=numItems;i++)
		{
			for(int x=0; x<=knapsackSize; x++)
			{
				long a = optimizedKnapsackCache[previousRow][x];
				
				int value = (int) (x-items[i-1][1]);
				long b = value<0?0:optimizedKnapsackCache[previousRow][value] +items[i-1][0];
				
				optimizedKnapsackCache[currentRow][x] = (a>b)?a:b;
			}
			//swap previous and current rows
			previousRow = previousRow==0?1:0;
			currentRow = currentRow==0?1:0;
		}
		
		//undo last unnecessary swap of rows.
		previousRow = previousRow==0?1:0;
		currentRow = currentRow==0?1:0;
	
		return optimizedKnapsackCache[currentRow][knapsackSize];
	}
}
