...
genotypeFactor.card = [genotypeCardinality];

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.
for genotypeId = allelesToGenotypes(:)'
	genotypeFactor.val(genotypeId) += alleleFreqs(genotypesToAlleles(genotypeId,1)) * alleleFreqs(genotypesToAlleles(genotypeId,2));
endfor
