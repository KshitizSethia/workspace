...
for index = 1:prod(phenotypeFactor.card)
	...
	genotypeAssignment = sort(assignment(1,2:3));
	genotypeIndex = find(sum(genotypeAssignment==genotypesToAlleles, 2)==2);
	
	if(assignment(1)==1)
		phenotypeFactor.val(index) = alphaList(genotypeIndex);
	else
		phenotypeFactor.val(index) = 1-alphaList(genotypeIndex);
	endif
endfor
