...
genotypeCardinality = nchoosek(numAlleles, 2) + numAlleles;
genotypeFactor.card = [genotypeCardinality, genotypeCardinality, genotypeCardinality];

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.

%for every row in genotypeFactor.val
for index = 1:length(genotypeFactor.val)
	
	genotypeValue = IndexToAssignment(index, genotypeFactor.card);
	
	%get parent and child alleles
	childAlleles	 = genotypesToAlleles(genotypeValue(1),:);
	parentOneAlleles = genotypesToAlleles(genotypeValue(2),:);
	parentTwoAlleles = genotypesToAlleles(genotypeValue(3),:);
	
	%form cartesian product of parent alleles
	cartproduct = cartprod(parentOneAlleles, parentTwoAlleles);
	
	probability = 0;
	%add 1/4 to probability if child is possible through each permutation
	for possibleChild = cartproduct'
		un = union(possibleChild, childAlleles);
		int = intersect(possibleChild, childAlleles);
		
		if(length(un) == length(int))
			probability+=0.25;
		endif
	endfor

	genotypeFactor.val(index) = probability;
endfor
