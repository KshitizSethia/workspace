...
for index = 1:numPeople
	hasParents = pedigree.parents(index, 1)!=0;
	
	if(hasParents)
		parent1 = pedigree.parents(index, 1);
		parent2 = pedigree.parents(index, 2);
		factorList(index) = childCopyGivenParentalsFactor(numAlleles, index, parent1, parent1+numPeople);
		factorList(index+numPeople) = childCopyGivenParentalsFactor(numAlleles, index+numPeople, parent2, parent2+numPeople);
	else
		factorList(index) = childCopyGivenFreqsFactor(alleleFreqs, index);
		factorList(index+numPeople) = childCopyGivenFreqsFactor(alleleFreqs, index+numPeople);
	endif
	
	phenotypeIndex=index+(2*numPeople);
	factorList(phenotypeIndex) = phenotypeGivenCopiesFactor(alphaList, numAlleles, index, index+numPeople, phenotypeIndex);
endfor
