...
for genotypeIndex = 1:numPeople
	if(pedigree.parents(genotypeIndex,1)==0)
		factorList(genotypeIndex) = genotypeGivenAlleleFreqsFactor(alleleFreqs, genotypeIndex);
	else
		factorList(genotypeIndex) = genotypeGivenParentsGenotypesFactor(numAlleles, genotypeIndex, pedigree.parents(genotypeIndex,1), pedigree.parents(genotypeIndex,2));
	endif
endfor

for phenotypeIndex = numPeople+1:2*numPeople
	factorList(phenotypeIndex) = phenotypeGivenGenotypeFactor(alphaList, phenotypeIndex-numPeople, phenotypeIndex);
endfor
