...
numParents = 2;
phenotypeFactor.card = [2,repmat(cellfun("length", alleleWeights), 1, numParents)];
...
for index = 1:prod(phenotypeFactor.card)
	...
	for geneIndex = 1:numGenes
		%geneIndex
		weight += alleleWeights{geneIndex}(assignment(1+geneIndex));
		weight += alleleWeights{geneIndex}(assignment(1+geneIndex+numGenes));
	endfor
	
	if(assignment(1)==1)
		phenotypeFactor.val(index) = computeSigmoid(weight);
	else
		phenotypeFactor.val(index) = 1-computeSigmoid(weight);
	endif
