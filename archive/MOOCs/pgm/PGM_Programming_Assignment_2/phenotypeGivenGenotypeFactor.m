...
phenotypeCardinality=2; % Either the phenotype will be expressed or not, so cardinality is 2.
genotypeCardinality=length(alphaList);
...
for alphaIndex = 1:genotypeCardinality
	phenotypeFactor.val = [phenotypeFactor.val, [alphaList(alphaIndex), 1-alphaList(alphaIndex)]];
endfor
